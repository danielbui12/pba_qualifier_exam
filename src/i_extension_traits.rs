...
pub fn ok_count(outcomes: Vec<Outcome>) -> usize {
	outcomes.into_iter().filter(|outcome| *outcome == Outcome::Ok).count()
}
pub fn something_went_wrong_count(outcomes: Vec<Outcome>) -> usize {
    // in case `PartialEq` is not allowed
    outcomes.into_iter().filter(|outcome| matches!(*outcome, Outcome::SomethingWentWrong)).count()
}
pub fn i_dont_know_count(outcomes: Vec<Outcome>) -> usize {
    // in case `PartialEq` is not allowed
    outcomes.into_iter().filter(|outcome| matches!(*outcome, Outcome::IDontKnow)).count()
}
...
impl OutcomeCount for Vec<Outcome> {
	...
	fn i_dont_know_count(&self) -> usize {
		i_dont_know_count(self.clone())
	}
	fn something_went_wrong_count(&self) -> usize {
		something_went_wrong_count(self.clone())
	}
}

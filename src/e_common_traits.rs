...
impl PartialOrd for Employee {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        ...
        let current_emp_value = self.experience / self.wage;
        let other_emp_value = other.experience / other.wage;

        if current_emp_value > other_emp_value {
...
impl TryFrom<String> for Employee {
	...
	fn try_from(value: String) -> Result<Self, Self::Error> {
		let information:  Vec<&str> = value.split(", ").collect();

        if information.len() > 4 {
            return Err("TooManyArgs".into());
        }

        Ok(Employee {
            name: String::from(information[0]),
            experience: information[1].parse().map_err(|_| "ValueTooBig".into())?,
            wage: information[2].parse().map_err(|_| "ValueTooBig".into())?,
            uid: information[3].parse().map_err(|_| "ValueTooBig".into())?,
        })
	}
}
...
impl From<Employee> for String {
	fn from(e: Employee) -> Self {
        let raw_e_data: Vec<String> = vec![
            e.name.to_string(),
            e.experience.to_string(),
            e.wage.to_string(),
            e.uid.to_string(),
        ];

...
pub fn bounded_absolute_values(vals: impl Iterator<Item = i32>) -> impl Iterator<Item = u32> {
        vals
            .map(|val| val.abs() as u32)
            .filter(|abs_val| *abs_val <= 100)
}
...
pub fn first_n_even(mut vals: impl Iterator<Item = u32>) -> Option<impl Iterator<Item = u32>> {
	let even_number_count = vals.next().unwrap();
    let even_number = vals.filter(|val| val % 2 == 0).take(even_number_count as usize);
    
    Some(even_number.into_iter())
}
...
pub fn square_whole_numbers() -> impl Iterator<Item = u32> {
	(0u32..).map(|val| val * val).into_iter()
}
    ...
    fn next(&mut self) -> Option<u32> {
        match (self.prev, self.prev_prev) {
            (None, None) => {
                ...
                self.prev // first call return 0
            },
            (Some(0), Some(0)) => {
                // init prev number
                self.prev = Some(1);
                self.prev // second call return 1
            },
            (Some(p), Some(p_p)) => {
                // calculate next number
                let next = p.checked_add(p_p);
                self.prev = next;
                self.prev_prev = Some(p);
                self.prev // from 2nd call return fibo number
            },
            _ => unreachable!("The iterator state is invalid"),
        }
    }
}
